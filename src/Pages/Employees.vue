<template>
      ...
      <div v-else>
        <form @submit.prevent="saveItem">
            <div class="form-group">
                ...
                <input v-model="selectedItem.login" type="text" id="io" required>
            </div>
            <div class="form-group">
  ...
  export default {
    ...
    methods: {
      ...
      edit(item) {
        get("employee",item.id).then(data => { 
      ...
      saveItem() {
        ...
        if(this.editMode) {
          put("employee", this.selectedItem.id ,this.selectedItem).then(res => {
            this.list();
            this.showGrid = true;
            this.showToast('Item saved successfully!', 'success');
          })
        }else {
          post("employee","",this.selectedItem).then(data => {  
            this.list();
            this.showGrid = true;
            this.showToast('Item saved successfully!', 'success');
          })
      ...
      list() {
        this.hash = {};
        list("employee").then(data => { this.items = data; });
